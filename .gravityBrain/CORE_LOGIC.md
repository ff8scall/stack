...
### 핵심 구현 로직
1. **멀티 엔드포인트 지원**: `src/lib/indexnow.ts`에서 Bing과 네이버 엔드포인트를 병렬로 호출합니다.
2. **스트리밍(Streaming) 모드**: 특정 브릭 ID(`ids`)를 파라미터로 받아 관련 URL만 선별 통보합니다. API 안정성을 위해 전송 URL 개수를 **1,000개로 제한(Limit)**하는 방어적 로직이 포함되어 있습니다.
3. **자동화 트리거**: GitHub Actions 완료 시점에 API를 호출하여 데이터 업데이트와 색인 요청을 동기화합니다.

---

## 10. Advanced SEO 및 AI Grounding 최적화

### 설계 의도
AI 모델이 웹을 검색하여 답변을 생성할 때, LegoStack의 정교한 비용 데이터를 정확하게 인용(Citation)하고 근거(Grounding)로 삼을 수 있도록 기계 판독이 용이한 구조를 제공하며, 구글 검색 결과에서의 시인성(Rich Snippets)을 극대화합니다.

### 핵심 로직
1. **다중 구조화 데이터 패키지**: 상세 페이지에 4종의 JSON-LD를 결합합니다.
   - `SoftwareApplication`: 모델의 가격, 별점, 카테고리 정보 제공.
   - `BreadcrumbList`: 사이트 계층 구조(Home > Category > Brick)를 명시하여 탐색 경로 최적화.
   - `FAQPage`: 가격 및 특징에 대한 질문/답변 데이터를 자동 생성하여 검색 결과 점유율 확대.
   - `Dataset`: 실시간으로 변동되는 가격 정보를 데이터셋으로 정의하여 검색 엔진의 신뢰도 확보.
2. **Silo 구조 및 내부 링크**: 메인 페이지 하단의 카테고리 실로(Silo) 링크와 상세 페이지의 연관 도구 비교 링크를 통해 봇의 크롤링 효율과 페이지 권위(Authority)를 높입니다.
3. **이미지 및 미디어 최적화**: 모든 이미지에 `loading="lazy"`를 적용하고, `next.config.ts` 설정을 통해 AVIF/WebP 포맷을 지원하여 Core Web Vitals 성능을 관리합니다.
